...
def getShopHoursEmbed(type, plan, estimated_time, sessionNumber):
    embed = Embed(
        ...
        fields=[
            EmbedField(name="Shop Type", value=f"{type}", inline=True),
            EmbedField(name="Rough Plan", value=f"{plan}", inline=True),
            EmbedField(name="Estimated Time", value=f"{estimated_time}", inline=True),
        ],
    )
    return embed
...
def get_leaderboard(people):
    ...
    for line in lines:
        ...
        if chunk_length > LEADERBOARD_LIMIT:
        ...
        chunk_length += 1
    embeds[-1].add_field("Sign out if you want time!!!", " ")
    return embeds
...
def _format_child_line(child: dict, *, tz: str) -> str:
    ...
    due_parsed, is_date_only = parse_due(due_raw, due_params, default_tz=tz)
    due_human = (
        humanize_due(due_parsed, is_date_only, display_tz=tz)
        if due_parsed
        else (due_raw or "—")
    )

    return (
        f"• {clamp(title, 80)} — {clamp(due_human, 60)} — {status or 'UNKNOWN-STATUS'}"
    )


def build_parent_task_embeds_grouped_by_list(
    by_uid: dict,
    children_of: dict,
    listname_of_uid: dict,
    *,
...
def build_parent_task_embed(
    parent: dict, children: list[dict], *, tz: str = "America/New_York"
) -> Embed:
    ...
    p_due_human = (
        humanize_due(p_due_parsed, p_is_date_only, display_tz=tz)
        if p_due_parsed
        else (p_due_raw or "—")
    )
